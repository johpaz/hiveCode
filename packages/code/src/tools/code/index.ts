...
const parseAstTool: Tool = {
  ...
  async execute(params) {
    ...
    try {
      ...
      // §39.5 — formatted summary map for efficient reading decisions
      const importSummary = importRecords.length
        ? importRecords.map(i => `  ${i.path} → ${(i.names ?? []).map((n: any) => n.name).join(", ") || "*"}`).join("\n")
        : "  (ninguno)";
      const exportSummary = exportRecords.length
        ? exportRecords.map(e => `  ${e.name} (${e.kind})`).join("\n")
        : "  (ninguno)";
      const funcSummary = functions.length ? functions.map(f => `  ${f}`).join("\n") : "  (ninguna)";
      const summary = [
        `[MAPA: ${filePath} — ${lines.length} líneas]`,
        `\nIMPORTS:\n${importSummary}`,
        `\nEXPORTS:\n${exportSummary}`,
        `\nFUNCIONES:\n${funcSummary}`,
        classes.length ? `\nCLASES:\n${classes.map(c => `  ${c}`).join("\n")}` : "",
        `\nCOMPLEJIDAD: ciclomática=${branchCount + 1} · ramas=${branchCount}`,
        lines.length > 100
          ? `\n→ Para leer una función: fs_read("${filePath}", offset=<línea-5>, limit=<tamaño+10>)`
          : "",
      ].filter(Boolean).join("");
