...
export class AgentService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      if (!this.explicitAgentId) {
        this.agentId = await resolveAgentId() || "main"
      }
  ...
  async getAgent(agentId?: string): Promise<AgentDBRecord | null> {
    ...
    return {
      id: entry.doc.id,
      user_id: entry.doc.user_id,
      name: entry.doc.name,
      description: entry.doc.description,
      system_prompt: entry.doc.system_prompt,
      tone: entry.doc.tone,
      role: entry.doc.role,
      status: entry.doc.status,
      enabled: entry.doc.enabled ? 1 : 0,
      provider_id: entry.doc.provider_id,
      model_id: entry.doc.model_id,
      tools_json: entry.doc.tools_json,
      skills_json: entry.doc.skills_json,
      parent_id: fromIndexable(entry.doc.parent_id),
  ...
  async getEthics(): Promise<string> {
    ...
    const active = (await ethics.scan()).map((entry) => entry.doc).find((doc) => doc.active)
