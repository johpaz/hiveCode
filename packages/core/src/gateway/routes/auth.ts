...
export async function generateTokens(userId: string): Promise<AuthTokens> {
  ...
  const refreshToken = await jwt.sign({ userId, type: "refresh", jti: crypto.randomUUID() }, JWT_SECRET, { expiresIn: "7d" });
...
export async function refreshAccessToken(refreshToken: string): Promise<AuthTokens | null> {
  const decoded = jwt.verify<{ userId: string; type: string; jti: string }>(refreshToken, JWT_SECRET);
...
export async function validateAccessToken(token: string): Promise<{ userId: string } | null> {
  const decoded = jwt.verify<{ userId: string; type: string }>(token, JWT_SECRET);
