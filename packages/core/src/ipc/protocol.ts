...
export type BunMessage =
  ...
  | { type: "file_risk_update"; path: string; risk: string; operation: string; adr_ref: string | null; reason: string; agent: string; lines_added?: number; lines_removed?: number }
  ...
  | { type: "file_diff";          path: string; branch?: string; stats_added?: number; stats_removed?: number; chunks: { kind: string; text: string; old_line_no?: number; new_line_no?: number }[] }
